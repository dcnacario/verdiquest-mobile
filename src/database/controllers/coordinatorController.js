...
const Coordinator = require('../models/coordinator');
    ...
    async function registerOrgnization(request, response){
        try {
            const {organizationName, organizationAddress, organizationType} = request.body;
            ...
            const insertedOrganizationId = await coordinator.insertOrganization(coordinatorData);
            ...
            response.status(200).send({ message: 'User registered successfully!', userId: insertedOrganizationId, token });
    ...
    async function registerCoordinator(request, response) {
        try {
            const { firstName, middleInitial, lastName, gender, phoneNumber, street,
                 barangay, city, province, username, password} = request.body;
